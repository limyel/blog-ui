    ...
    <div class="toc-wrapper">
      <ul class="toc">
        ...
        <li v-for="(h2, index) in titles" :key="index">
          <span @click="scrollToView(h2.offsetTop)" class="py-2 pl-5 cursor-pointer">{{ h2.text }}</span>
          <!-- 三级标题 -->
          <ul v-if="h2.children && h2.children.length > 0">
            <li v-for="(h3, index2) in h2.children" :key="index2">
              <span @click="scrollToView(h3.offsetTop)" class="pl-10 cursor-pointer">{{ h3.text }}</span>
            </li>
          </ul>
        </li>
      </ul>
